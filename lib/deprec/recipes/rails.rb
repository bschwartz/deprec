...
Capistrano::Configuration.instance(:must_exist).load do 
  ...
  namespace :deprec do
    namespace :rails do
      ...
      set :db_name, lambda { "#{application}_#{rails_env}" }
      set :db_user, lambda { application }
      set :db_pass, lambda { application } # password is the same as the user
...
#{rails_env}:
  ...
  host: #{db_host}
EOF
        database_configuration << "socket: #{db_socket}" if db_socket
